	...
	$(document).ready(function() {
		...
		$('#btnRegister').on('click', function(){
			...
			if(userId.match(regExp) == null){
				alert("유효하지 않은 아이디 양식입니다.\n6~12자리의 길이를 가지는 영문 숫자 _ 만을 사용하세요");
				joinForm.memberId.focus();
				return;
			} 
		...
		//	}
			...
			regExp = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{3,4}$/i;
			if(email.match(regExp) == null){
				alert("유효하지 않은 이메일 양식입니다. \nEX)sample@sample.xxxx");
				$('#email').focus();
				return;
			}
		...
		$('#btnHome').on('click', function(){
			document.loginForm.action="<c:url value='/index.do'/>";
...
/*	function doJoin(){
		...
		joinForm.action = "spring_pf_01/member/join.do";
		joinForm.method = "POST";
		joinForm.submit(); // FORM 태그에서 action하는 것이 아닌 유효성 검사후 sub하도록 설정.
	}*/
</script>
	...
	<div class="form">
		...
		<div class="form-panel one">
			...
			<div class="form-content">
				<form action="/spring_pf_01/member/login.do" method="post" name="loginForm">
					<div class="form-group">
						<label for="username">User Id</label> 
						<input type="text" id="" name="memberId" required="required" value="${memberId}"/>
					</div>
					<div class="form-group">
						<label for="password">Password</label> 
						<input type="password" id="" name="memberPw" required="required" value="${memberPw}"/>
					</div>
					<br/><br/>
					<div class="form-group">
		...
		<div class="form-panel two">
			...
			<div class="form-content">
				...
				<!--<div class="form-group">
						<label for="nickname">Birth Date(yyyy-mm-dd)</label> 
						<input type="text" id="" name="birthDate" required="required" />
					</div> -->
					<div class="form-group">
